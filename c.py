...
def main():
    ...
    print(f"Please enter details for up to {max_entries} recipients.")
    print("Leave the email blank at any point to finish input early.\n")

    for i in range(max_entries):
        to_email = input(f"Entry {i+1} - Enter recipient's email address: ").strip()
        if not to_email:
            print("Input finished early.\n")
            break
        name = input(f"Entry {i+1} - Enter recipient's name (leave blank for HR Team): ").strip()
        company = input(f"Entry {i+1} - Enter company name: ").strip()
        if not company:
            print("Company name is required. Please re-enter this entry.\n")
            continue
        entries.append((to_email, name, company))
        print()

    print(f"Sending emails for {len(entries)} recipient(s)...\n")
    for to_email, name, company in entries:
